		...
		<div class="main">
			...
			<div class="pageColumn">
				<div style="font-size: 14px;">
					需求密码锁：
					<s:property value="keyAsk.kindOfKey.kindName" />
					<br /> 需求时间：
					<s:date name="keyAsk.askDate" format="yyyy-MM-dd" />
					<br /> 申请人：
					<s:property value="keyAsk.managerByCreateBy.name" />
					(
					<s:property value="keyAsk.managerByCreateBy.userName" />
					)<br /> 还需满足个数：<span id="lastNum"><s:property
							value="lastNum" />
					</span>
				</div>
								...
								<s:iterator value="CountObjectList">
									<s:if test="num>0">
										<tr>
											...
											<td><s:textfield cssClass="shipmentNum"
													name="shipmentNum" theme="simple" maxLength="6" /> <s:hidden
													name="lotNumber" value="%{item}" /></td>
										</tr>
									</s:if>
								</s:iterator>
								<tr>
									<td colspan="3"><input type="submit" value="提交"
										class="button2" /> <input type="button" id="rollBack"
										value="返回" class="button2" /></td>
								</tr>
							</tbody>
						</table>
